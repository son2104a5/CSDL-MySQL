...
select
	p.product_name, w.warehouse_name, i.quantity
from inventory i
join products p on p.product_id = i.product_id
join warehouses w on w.warehouse_id = i.warehouse_id
where p.category = 'Electronics'
order by i.quantity desc limit 3 offset 1;

select
	p.product_name, s.supplier_name, w.warehouse_name, i.quantity
from inventory i
join products p on p.product_id = i.product_id
join warehouses w on w.warehouse_id = i.warehouse_id
join suppliers s on s.product_id = p.product_id
where p.price > 5000000
order by p.product_name asc, s.supplier_name desc;

select
	p.product_name, p.price, w.warehouse_name, w.location, i.quantity, s.supplier_name
from products p
join inventory i on p.product_id = i.product_id
join warehouses w on w.warehouse_id = i.warehouse_id
join suppliers s on s.product_id = p.product_id
where p.category = 'Electronics' or p.category = 'Fashion'
order by i.quantity desc limit 5;

select
	p.product_name, p.price, w.warehouse_name, i.quantity, s.supplier_name, p.category
from products p
join inventory i on p.product_id = i.product_id
join warehouses w on w.warehouse_id = i.warehouse_id
join suppliers s on s.product_id = p.product_id
where s.supplier_name = 'Tech Corp' or s.supplier_name = 'Mobile World' and p.price > 5000000
order by p.price limit 4;